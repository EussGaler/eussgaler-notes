...
> #### 例
>
> <img src="assets/chapt_03/3-02-单位负反馈稳态误差例题.png" alt="单位负反馈稳态误差例题" width="560" style="display:block;max-width:100%;height:auto;margin:1.2em auto;">
>
> 为单位反馈系统，故
>
> $$
...
> 1. 系统稳定。
> 2. 按输入端定义误差。
> 3. $r(t)$ 作用，且 $r(t)$ 无其他前馈通道，如：
>
> <img src="assets/chapt_03/3-03-含输入前馈通道的系统.png" alt="含输入前馈通道的系统" width="700" style="display:block;max-width:100%;height:auto;margin:1.2em auto;">
...
|  |  | 静态误差系数 |  |  | 稳态误差计算 |  |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: |
| 型别 | $K_p=\lim\limits_{s\to0}G(s)H(s)$<br>$=\lim\limits_{s\to0}\dfrac{K}{s^\nu}$ | $K_v=\lim\limits_{s\to0}sG(s)H(s)$<br>$=\lim\limits_{s\to0}\dfrac{K}{s^{\nu-1}}$ | $K_a=\lim\limits_{s\to0}s^2G(s)H(s)$<br>$=\lim\limits_{s\to0}\dfrac{K}{s^{\nu-2}}$ | $r=A\cdot1(t)$<br>$e_{ss}=\dfrac{A}{1+K_p}$ | $r=A\cdot t$<br>$e_{ss}=\dfrac{A}{K_v}$ | $r=A\cdot\dfrac{t^2}{2}$<br>$e_{ss}=\dfrac{A}{K_a}$ |
| 0 | $K$ | $0$ | $0$ | $\dfrac{A}{1+K}$ | $\infty$ | $\infty$ |
| Ⅰ | $\infty$ | $K$ | $0$ | $0$ | $\dfrac{A}{K}$ | $\infty$ |
| Ⅱ | $\infty$ | $\infty$ | $K$ | $0$ | $0$ | $\dfrac{A}{K}$ |
