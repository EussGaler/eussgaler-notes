...
> **【图片占位 3-01】** 非单位负反馈系统：$R(s)$、$E(s)$、$G(s)$、$C(s)$、$H(s)$、$B(s)$，相加点负反馈。
...
> **【图片占位 3-02】** 左侧例题：前向通道 $K/[s(Ts+1)]$ 的单位负反馈系统，输入 $R(s)$、偏差 $E(s)$、输出 $C(s)$。
...
> **【图片占位 3-03】** 左侧“其他前馈通道”示意：$r(t)$ 经前馈方框与误差信号相加，进入 $K/[s(Ts+1)]$；保留原图 $G_c(s)$、$e(t)$、$u(t)$、$c(t)$ 标记。
...
> **【图片占位 3-04】** 左侧“几种情形”的六幅根轨迹示意；保留实轴零、极点、圆弧、分离／汇合位置与箭头方向。
...
> **【图片占位 3-05】** 渐近线交点 $\sigma_a$ 与倾角 $\varphi_a$ 示意。
>
> **【图片占位 3-06】** 根轨迹小例图：保留实轴零、极点、上下共轭的两条轨迹、竖直虚线及分离点 $d=-0.55$ 标注；后续重绘时对照原图核验点位，不能把分离点误记为渐近线交点。
